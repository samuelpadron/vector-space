...
import kornia.geometry.conversions as KConversions
...
class DisplacementHead(nn.Module):
    """Represents H0: Non-rigid, per-pixel alignment (AlignNet proxy)"""
    def __init__(self, camera_channels=256, lidar_channels=1):
        super().__init__()
        mid_channels = 128
        self.net = nn.Sequential( # See Figure 4 of CRFusion paper for reference
            nn.Conv2d(camera_channels + lidar_channels, mid_channels, kernel_size=1),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, 2, kernel_size=3, padding=1),
            nn.Tanh() # Offset constraint 
        )
...
def create_lidar_bev(nusc, sample_token, grid_conf):
    # Follow PointPillars-style BEV creation for LIDAR data
    sample = nusc.get('sample', sample_token)
    lidar_data = nusc.get('sample_data', sample['data']['LIDAR_TOP'])
    lidar_path = Path(nusc.dataroot) / lidar_data['filename']
    
    # load point cloud
    points = np.fromfile(lidar_path, dtype=np.float32).reshape(-1, 5)[:, :4]  # (N, 4) [x, y, z, intensity]
    ...
    # Initialize BEV grid
    bev_grid = np.zeros((x_size, y_size), dtype=np.float32)

    # Project points to BEV grid
    mask = (points[:, 0] >= x_min) & (points[:, 0] <= x_max) & (points[:, 1] >= y_min) & (points[:, 1] <= y_max)
    pts = points[mask]
    xi = ((pts[:, 0] - x_min) / x_res).astype(int).clip(0, x_size - 1)
    yi = ((pts[:, 1] - y_min) / y_res).astype(int).clip(0, y_size - 1)
    bev_grid[yi, xi] = 1.0

    return torch.from_numpy(bev_grid).unsqueeze(0).unsqueeze(0)  # (1, 1, H, W)


def apply_dense_warp(features, delta):
    """
    features: [B, C, H, W]
    delta: [B, 2, H, W] (offsets in pixel units)
    """
    ...
    # Normalize Delta to the [-1, 1] range
    # Pixel delta / (size/2) converts to normalized units
    normalized_delta = torch.stack([
        delta[:, 0, :, :] / (W / 2),
        delta[:, 1, :, :] / (H / 2)
    ], dim=-1)
    
    warped_grid = grid + normalized_delta
    return F.grid_sample(features, warped_grid, align_corners=False, mode='bilinear')


def run_geometric_test(delta, H, W):
    """
    Fits a Sim(2) model to a dense displacement field using Kornia Lie Groups
    and calculates the Geodesic Distance on the manifold.
    """
    device = delta.device
    
    # 1. Generate normalized source grid [-1, 1]
    # Using create_meshgrid directly as requested
    grid = create_meshgrid(H, W, normalized_coordinates=True, device=device)
    src_pts = grid.reshape(-1, 2).double()
    
    # 2. Normalize the H0 displacement field from pixels to [-1, 1] space
    norm_delta = torch.stack([
        delta[0, 0] / (W / 2),
        delta[0, 1] / (H / 2)
    ], dim=-1).reshape(-1, 2).double()
    
    dst_pts = src_pts + norm_delta

    # 3. Solve for Similarity Matrix via Least Squares (Sim2)
    x, y = src_pts[:, 0], src_pts[:, 1]
    num_pts = x.shape[0]
    
    A = torch.zeros((2 * num_pts, 4), device=device, dtype=torch.float64)
    A[0::2, 0], A[0::2, 1], A[0::2, 2] = x, -y, 1.0 # a*x - b*y + tx
    A[1::2, 0], A[1::2, 1], A[1::2, 3] = y,  x, 1.0 # b*x + a*y + ty
    B = dst_pts.reshape(-1, 1)

    sol = torch.linalg.lstsq(A, B).solution.squeeze()
    a, b, tx, ty = sol
    
    # 4. Map to Lie Group primitives using KLieGroup
    scale = torch.sqrt(a**2 + b**2)
    # Construct normalized rotation matrix (SO2)
    rot_mat = torch.stack([
    ...
    theta_rad = so2_element.log().squeeze() # Extract rotation from Lie Algebra
    
    # 5. Geodesic Distance on the SE(2) Manifold
    # Represents the shortest path distance in the tangent space
    t_vec = torch.tensor([[tx, ty]], device=device, dtype=torch.float64)
    se2_element = KLieGroup.Se2(so2_element, t_vec)
    
    # The norm of the log map provides the geodesic distance
    geodesic_dist = torch.norm(se2_element.log()).item()

    # 6. Calculate residuals and R2
    fitted_pts = (A @ sol).reshape(-1, 2)
    residuals = dst_pts - fitted_pts
    
    ss_res = (residuals ** 2).sum()
    ss_tot = ((norm_delta - norm_delta.mean()) ** 2).sum()
    ...
    return params, r2_score.item(), residuals.reshape(H, W, 2).float()

    
def visualize_geometry_comparison(delta, params, sample_idx, save_path=None):
    """
    Visualizes the Duel:
    Red = The full, unconstrained dense flow (H0 - CRFusion style)
    Blue = The rigid Sim(2) approximation (H1 - Lie Group style)
    """

    device = delta.device
    # final_delta is [1, 2, H, W], we want [H, W, 2]
    dense_flow = delta[0].permute(1, 2, 0).cpu().numpy()
    ...
    # Calculate components of the Sim(2) matrix from parameters
    rad = np.deg2rad(params['theta_deg'])
    a = params['scale'] * np.cos(rad)
    b = params['scale'] * np.sin(rad)
    
    # Apply Sim(2) Transform: 
    # x' = ax - by + tx 
    # y' = bx + ay + ty
    fit_x = a * grid[:, 0] - b * grid[:, 1] + params['tx']
    fit_y = b * grid[:, 0] + a * grid[:, 1] + params['ty']
    
    # Displacement = Fitted_Target - Source_Grid
    rigid_flow_flat = torch.stack([fit_x - grid[:, 0], fit_y - grid[:, 1]], dim=-1)
    # Scale normalized displacement back to pixel units for quiver comparison
    rigid_flow = rigid_flow_flat.view(H, W, 2).cpu().numpy()
    rigid_flow[..., 0] *= (W / 2)
    rigid_flow[..., 1] *= (H / 2)

    # Setup Plot
    step = 8  # Subsample for visual clarity
    plt.figure(figsize=(12, 12))
    
    # Coordinate grids for the quiver plot
    X, Y = np.meshgrid(np.arange(0, W, step), np.arange(0, H, step))
    
    # Dense Flow (H0) in Red
    plt.quiver(X, Y, 
               dense_flow[::step, ::step, 0], 
               dense_flow[::step, ::step, 1], 
               color='red', alpha=0.4, label='H0: Dense Flow (Learned)', 
               angles='xy', scale_units='xy', scale=1.0)
    
    # Plot Fitted Sim2 (H1) in Blue
    plt.quiver(X, Y, 
    ...
    # Formatting
    plt.title(f"Hypothesis Test (Sample {sample_idx}): H0 (Dense) vs H1 (Rigid)\n"
              f"R²: {params['r2']:.4f} (Fit Quality) | Geodesic Dist: {params['geodesic_dist']:.4f} (Error Mag)\n"
              f"Sim(2): Rot={params['theta_deg']:.2f}° | Trans=[{params['tx']:.2f}, {params['ty']:.2f}]")
    plt.legend(loc='upper right')
    plt.grid(alpha=0.2)
    plt.gca().invert_yaxis() # Match BEV/Image coordinate system
...
def visualize_residual_heatmap(residuals, sample_idx, save_path=None):
    """
    Heatmap of non-rigid error. 
    Dark = Sim(2) explains the movement well.
    Bright = Large non-rigid distortion.
    """
    # residuals is [H, W, 2]
    ...
    plt.colorbar(label='Pixel Error Magnitude (Normalized Units)')
    plt.title(f'Residual Heatmap (Sample {sample_idx}): Where H1 Fails')
    
    if save_path:
        plt.savefig(save_path, bbox_inches='tight')
        print(f"  Saved residual heatmap to {save_path}")
    plt.close()
...
def main():
    ...
    # Initiate displacement head (H0) to predict dense pixel-wise displacements for alignment (proxy for AlignNet)
    displacement_head = DisplacementHead(camera_channels=256, lidar_channels=1).to(device)

    # match output shape of fastbev backbone
    ...
    for sample_idx in range(min(3, len(nusc.sample))):
        ...
        with torch.no_grad():
            ...
            cam_bev = outputs['bev_feat']
            
        lidar_bev = create_lidar_bev(nusc, sample_token, grid_conf).to(device)
        
        # Ensure LiDAR matches Camera dimensions exactly
        if lidar_bev.shape[-2:] != cam_bev.shape[-2:]:
            print(f"  Resizing LiDAR BEV from {lidar_bev.shape[-2:]} to {cam_bev.shape[-2:]} for alignment...")
            lidar_bev = F.interpolate(lidar_bev, size=cam_bev.shape[-2:], mode='bilinear')
        
        # Optimize displacement head to find the 'best' unconstrained alignment
        print(f"  Optimizing DisplacementHead for sample {sample_idx}...")
        optimizer = torch.optim.Adam(displacement_head.parameters(), lr=1e-2)
        
        for i in range(200): # Mini-optimization loop
            optimizer.zero_grad()
            ...
            # Use mean of features as occupancy proxy
            cam_occ = warped_cam.mean(dim=1, keepdim=True)
            
            # How well does the warped camera feature match LiDAR?
            loss = F.mse_loss(cam_occ, lidar_bev)
            loss.backward()
            optimizer.step()
        ...
        # Fit Lie parameters [tx, ty, theta, scale]
        params, r2_score, residuals = run_geometric_test(final_delta, H, W)
        print(f"  Geometric Test Results for sample {sample_idx}:")
        print(f" [Result] R2 Score: {r2_score:.4f}")
        print(f" [Result] Predicted Sim(2) Parameters: tx={params['tx']:.3f}, ty={params['ty']:.3f}, theta={params['theta_deg']:.2f} deg, scale={params['scale']:.4f}")

        # Visualize
        visualize_geometry_comparison(final_delta, params, sample_idx, save_path= os.path.join(output_dir, "displacement", f"sample_{sample_idx}_geometry_comparison.png"))

        visualize_residual_heatmap(residuals, sample_idx, save_path=os.path.join(output_dir, "heatmaps", f"sample_{sample_idx}_residual_heatmap.png"))
        
        # Reset displacement head for next sample
        displacement_head = DisplacementHead(camera_channels=256, lidar_channels=1).to(device)
