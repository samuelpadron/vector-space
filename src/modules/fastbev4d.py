...
Temporal Fusion version (ala bevdet4d) with ego-motion compensation and BEV warping.
"""
...
from modules.ego_motion import EgoMotionEstimator
from modules.temporal_fusion import BEVWarp, BEVTemporalFusionConcat, compute_se2_from_ego_poses
...
class FastBEV(nn.Module):
    ...
    def forward(self, img, cam2ego, cam_intrinsics, img_aug_matrix=None):
        ...
        # Temporal fusion
        bev_feat = BEVWarp.apply(bev_feat, cam2ego, depth)
...
def compute_se2_from_ego_poses(ego_pose_prev: dict, ego_pose_curr: dict, grid_resolution: float = 0.8) -> Dict:
    """
    Compute SE(2) transform between two ego poses (Phase 1: Ego-Motion Module).

    Args:
        ego_pose_prev: nuScenes ego_pose dict from t-1
        ego_pose_curr: nuScenes ego_pose dict from t
        grid_resolution: Meters per BEV grid cell

    Returns:
        Dict with 'dx', 'dy', 'dyaw' in grid units, plus meter values
    """
    ...
    # Yaw delta from quaternions
    yaw_prev = q_prev.yaw_pitch_roll[0]
    yaw_curr = q_curr.yaw_pitch_roll[0]
    ...
    dx_grid = dx_m / grid_resolution
    dy_grid = dy_m / grid_resolution
...
def load_sample(nusc, sample_token, target_size=(256, 704)):
    """Load a nuScenes sample with monocamera (CAM_FRONT) and ego_pose."""
    ...
    # Get ego pose for temporal fusion
    lidar_sd = nusc.get('sample_data', sample['data']['LIDAR_TOP'])
    ego_pose_dict = nusc.get('ego_pose', lidar_sd['ego_pose_token'])
    ego_pose = {
        'translation': np.array(ego_pose_dict['translation']),
        'rotation': Quaternion(ego_pose_dict['rotation']),
    }

    return img_tensor, intrinsics, cam2ego, img_aug, sample, ego_pose
...
def main():
    ...
    for sample_idx in range(min(3, len(nusc.sample))):
        ...
        # Load data
        images, intrinsics, cam2egos, img_aug_matrices, sample_data, ego_pose = load_sample(nusc, sample_token)
        ...
        # Get t-1 bev features and align them with current timestep t
        if sample_idx > 0:
            sample_prev = nusc.sample[sample_idx - 1]
            img_prev, intrinsics_prev, cam2egos_prev, img_aug_matrices_prev, _, ego_pose_prev = load_sample(nusc, sample_prev['token'])
            se2 = compute_se2_from_ego_poses(ego_pose_prev, ego_pose)
            print(f"  SE(2) ego-motion: Δx={se2['dx']:.3f} grid_px, Δy={se2['dy']:.3f} grid_px, Δyaw={np.degrees(se2['dyaw']):.2f}°")
            print(f"               ({se2['dx_m']:.4f}m, {se2['dy_m']:.4f}m)")

            # feed model bev features from previous timestep + SE(2) for temporal fusion
            with torch.no_grad():
                bev_feat_prev = model(img_prev.unsqueeze(0).to(device), cam2egos_prev.unsqueeze(0).to(device), intrinsics_prev.unsqueeze(0).to(device), img_aug_matrices_prev.unsqueeze(0).to(device))['bev_feat']  

            bev_feat_aligned = BEVWarp.apply(bev_feat_prev, se2['dx'], se2['dy'], se2['dyaw'])
